...
def configure(context):
    context.stage("synthesis_thailand.population.spatial.primary.candidates")
    context.stage("synthesis_thailand.population.spatial.commute_distance")
...
def process_municipality(context, origin_id):
    ...
    df_candidates = df_candidates.rename(columns = dict(destination_id = "zone"))

    return df_candidates[["person_id", "zone", "location_id", "geometry"]]
...
def execute(context):
    data = context.stage("synthesis_thailand.population.spatial.primary.candidates")
    ...
    df_commute_distance = context.stage("synthesis_thailand.population.spatial.commute_distance")
    ...
    # Attach geometry
    df_locations = context.stage("synthesis_thailand.locations.work")[["location_id", "geometry"]]
    ...
    df_locations = context.stage("synthesis_thailand.locations.education")[["location_id", "geometry"]]
