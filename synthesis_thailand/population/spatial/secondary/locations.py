...
from synthesis_thailand.population.spatial.secondary.problems import find_assignment_problems

def configure(context):
    context.stage("synthesis_thailand.population.trips")
    ...
    context.stage("synthesis_thailand.population.spatial.secondary.distance_distributions")
    ...
    context.stage("synthesis_thailand.locations.secondary")
...
def prepare_locations(context):
    ...
    df_locations = context.stage("synthesis_thailand.population.sampled")[["person_id", "household_id"]]
...
def prepare_destinations(context):
    df_locations = context.stage("synthesis_thailand.locations.secondary")
...
from synthesis_thailand.population.spatial.secondary.rda import AssignmentSolver, DiscretizationErrorObjective, GravityChainSolver, AngularTailSolver, GeneralRelaxationSolver
from synthesis_thailand.population.spatial.secondary.components import CustomDistanceSampler, CustomDiscretizationSolver, CandidateIndex, CustomFreeChainSolver

def execute(context):
    # Load trips and primary locations
    df_trips = context.stage("synthesis_thailand.population.trips").sort_values(by = ["person_id", "trip_index"])
    df_trips["travel_time"] = df_trips["arrival_time"] - df_trips["departure_time"]
    df_primary = prepare_locations(context)

    # Prepare data
    distance_distributions = context.stage("synthesis_thailand.population.spatial.secondary.distance_distributions")
    destinations = prepare_destinations(context)

    # Resampling for calibration
    resample_distributions(distance_distributions, dict(
        car = 0.0, car_passenger = 0.1, pt = 0.5, bike = 0.0, walk = -0.5, stay=-1
    ))
...
def process(context, arguments):
  ...
  thresholds = dict(
    ...
    bike = 100.0, walk = 100.0,  stay= 50.0
  )
