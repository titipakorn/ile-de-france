...
def execute(context):
    ...
    df_target = context.stage("synthesis_thailand.population.sampled")

    df_source_households, df_source_persons, df_source_trips = context.stage("data.hts.thailand.cleaned")
    ...
    if "income_class" in df_source:
        df_target["income_class"] = np.digitize(df_target["income"], INCOME_BOUNDARIES, right = True)
    ...
    df_source = df_source[["hts_id","household_id","person_weight","age","sex","employed","studies","P_TOTALTRIP","departement_id","household_weight","household_size","number_of_vehicles","number_of_bikes",'income_class',"age_class","any_cars"]]
    df_target = df_target[["person_id","household_id","departement_id","age","sex","couple","employed","studies","number_of_vehicles","household_size","age_class","income_class","any_cars"]]
    # return [df_source, df_target, columns]
