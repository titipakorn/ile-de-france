...
def configure(context):
    ...
    # if need_osm:
    #     context.stage("matsim_thailand.scenario.supply.osm")
...
def execute(context):
    ...
    for name in file_names:
        shutil.copy(
            "%s/%s" % (context.path("matsim_thailand.simulation.prepare"), name),
            "%s/%s" % (context.config("output_path"), name)
        )

    # if context.config("export_detailed_network"):
    #     shutil.copy(
    #         "%s/%s" % (context.path("matsim_thailand.scenario.supply.osm"), "detailed_network.csv"),
    #         "%s/%s" % (context.config("output_path"), "%sdetailed_network.csv" % context.config("output_prefix"))
    #     )
    
    if context.config("write_jar"):
        shutil.copy(
            "%s/%s" % (context.path("matsim_thailand.runtime.eqasim"), context.stage("matsim_thailand.runtime.eqasim")),
