...
def configure(context):
    context.stage("matsim_thailand.scenario.population")
    context.stage("matsim_thailand.scenario.households")

    if context.config("generate_vehicles_file", False):
        context.stage("matsim_thailand.scenario.vehicles")

    context.stage("matsim_thailand.scenario.facilities")
    # context.stage("matsim_thailand.scenario.supply.processed")
    # context.stage("matsim_thailand.scenario.supply.gtfs")
...
def execute(context):
    # Prepare input files
    facilities_path = "%s/%s" % (
        context.path("matsim_thailand.scenario.facilities"),
        context.stage("matsim_thailand.scenario.facilities")
    )

    population_path = "%s/%s" % (
        context.path("matsim_thailand.scenario.population"),
        context.stage("matsim_thailand.scenario.population")
    )

    network_path = "%s/%s" % (
        context.config("data_path"),
        "thailand_supply/BK_NW_PT_01.xml"
    )
    ...
    households_path = "%s/%s" % (
        context.path("matsim_thailand.scenario.households"),
        context.stage("matsim_thailand.scenario.households")
    )
    ...
    transit_schedule_path = "%s/%s" % (
       context.config("data_path"),
       "thailand_supply/schedule_m01.xml.gz"
    )
    ...
    transit_vehicles_path = "%s/%s" % (
        context.config("data_path"),
        "thailand_supply/vehicle_m01.xml.gz"
    )
    ...
    if context.config("generate_vehicles_file"):
        vehicles_path = "%s/%s" % (
            context.path("matsim_thailand.scenario.vehicles"),
            context.stage("matsim_thailand.scenario.vehicles")
        )
