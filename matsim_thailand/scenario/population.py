...
def execute(context):
    ...
    df_activities = context.stage("synthesis_thailand.population.activities").sort_values(by = ["person_id", "activity_index"])
    df_locations = context.stage("synthesis_thailand.population.spatial.locations")[[
    ...
    #filter no geometry
    df_persons=df_persons[~df_persons['person_id'].isin(df_activities[df_activities['geometry']==None]['person_id'].values)]
    df_trips=df_trips[~df_trips['person_id'].isin(df_activities[df_activities['geometry']==None]['person_id'].values)]
    df_activities=df_activities[~df_activities['person_id'].isin(df_activities[df_activities['geometry']==None]['person_id'].values)]

    # print("LENGTH", len(df_persons))
    p_count=0
    a_missing=[]
            ...
            with context.progress(total = len(df_persons), label = "Writing population ...") as progress:
                for person in df_persons.itertuples(index = False):
                    ...
                    if len(activities) > 0:
                        ...
                        if len(trips) == len(activities) - 1:
                            add_person(writer, person, activities, trips)
                            p_count+=1
                    else:
                        a_missing.append(person_id)
    ...
    # print('WHY??',len(df_persons),p_count,"A MISSING:",a_missing)
